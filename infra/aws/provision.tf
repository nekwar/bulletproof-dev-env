...
module "provision" {
  ...
  nodegroups = { for k, ngd in local.nodegroups_wplatform : k => {
    ...
    instance_profile_name : (ngd.role == "controller+worker" || ngd.role == "controller") ? aws_iam_instance_profile.controller_iam_instance_profile.name : aws_iam_instance_profile.worker_iam_instance_profile.name
  } }
